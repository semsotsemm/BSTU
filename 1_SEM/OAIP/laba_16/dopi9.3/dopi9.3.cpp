#include <iostream>
using namespace std;

void diag(int number, int** A, int N, int M)
{
	int di;
	cout << "Введите массив" << endl;
	for (int i = 0; i < N; i++)
		for (int j = 0; j < M; j++)
		{
			cout << "Введите элемент массива ";
			cin >> A[i][j];
			if (number == A[i][j] && i == j)
				di = 1;
		}
	for (int i = 0; i < N; i++)
	{
		cout << endl;
		for (int j = 0; j < M; j++)
			cout << A[i][j] << "\t";
	}
	if (di == 1)
		cout << endl << "number находится на главной диагонали";
	else
		cout << endl << "number не находится на главной диагонали";
	for (int i = 0; i < N; i++)
		delete A[i];
	delete[] A;
	return;
}

void firstsymb(int n, int len, int** mas)
{
	for (int i = 0; i < n; i++)
		for (int j = 0; j < len; j++)
			mas[i][j] = rand() % 25 + 97;
	for (int i = 0; i < n; i++)
		if (mas[i][0] == 'b')
		{
			for (int j = 0; j < len; j++)
			{
				char b = mas[i][j];
				cout << b;
			}
			cout << endl;
		}
	for (int i = 0; i < n; i++)
	delete mas[i];
	delete[] mas;
	return;
}

int main()
{
	int N, M, number = 0, ** A,choose, n, len,**mas;
	setlocale(LC_CTYPE, "Russian");
	do
	{
		cout << endl << "Выберите вариант работы программы\n";
		cout << "1 - проверить находится ли элемент 2 мерного массива на главное диагонали \n";
		cout << "2 - найти слова которые начинаются с b\n";
		cout << "3 - выход \n";
		cin >> choose;
		switch (choose)
		{
		case 1:
			cout << "Введите число строк ";
			cin >> N;
			cout << "Введите чиcло столбцов ";
			cin >> M;
			A = new int* [N];
			for (int i = 0; i < N; i++)
				A[i] = new int[M];
			cout << "Введите число number ";
			cin >> number;
			diag(number, A, N, M);
			break;
		case 2:
			cout << "Введите количество строк ";
			cin >> n;
			cout << "Введите длинну 1 строки ";
			cin >> len;
			mas = new int* [n];
			for (int i = 0; i < n; i++)
				mas[i] = new int[len];
			firstsymb(n, len, mas);
			break;
		case 3:
			break;
		}
	} while (choose != 3);
}